...
export function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const handleContinue = () => {
    if (selectedIntegrations.size > 0) {
      onLogin(Array.from(selectedIntegrations));
    }
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-8 py-16">
      ...
      >
        ...
        <div className="space-y-6">
          <h1 className="text-5xl md:text-6xl tracking-tight" style={{ 
          ...
          <p className="text-2xl md:text-3xl tracking-wide" style={{ 
        ...
        >
          <button
            ...
            style={{
              backgroundColor: selectedIntegrations.has('github') ? 'var(--zen-sage)' : 'white',
              color: selectedIntegrations.has('github') ? 'white' : 'var(--zen-charcoal)',
              border: selectedIntegrations.has('github') ? 'none' : '2px solid var(--zen-sage)'
            }}
          >
          ...
          <button
            ...
            style={{
              backgroundColor: selectedIntegrations.has('slack') ? 'var(--zen-blue)' : 'white',
              color: selectedIntegrations.has('slack') ? 'white' : 'var(--zen-charcoal)',
              border: selectedIntegrations.has('slack') ? 'none' : '2px solid var(--zen-blue)'
            }}
          >
            <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="w-full max-w-md mx-auto flex items-center justify-between gap-3 px-8 py-4 rounded-full transition-all duration-300 hover:shadow-lg"
            style={{
              backgroundColor: selectedIntegrations.has('linear') ? 'var(--zen-purple)' : 'white',
              color: selectedIntegrations.has('linear') ? 'white' : 'var(--zen-charcoal)',
              border: selectedIntegrations.has('linear') ? 'none' : '2px solid var(--zen-purple)'
            }}
          >
            <div className="flex items-center gap-3">
          ...
          >
            Continue with {selectedIntegrations.size} integration{selectedIntegrations.size !== 1 ? 's' : ''}
          </motion.button>
        </motion.div>
        ...
        >
          Your data stays private. We only track work activity to help you shine.
        </motion.p>
      </motion.div>
    </div>
  );
